...
class DatabaseProvider with ChangeNotifier {
  ...
  Future<double> getTotalExpense() async {
    final db = await database;
    var result = await db.rawQuery("SELECT SUM(amount) as total FROM expenseTable");
  ...
  Future<double> getExpenseByCategory(String category) async {
    ...
    var result = await db.rawQuery(
        "SELECT SUM(amount) as total FROM expenseTable WHERE category = ?", [category]);
    return result[0]["total"] != null ? (result[0]["total"] as double) : 0.0;
  }

  // Get expense for the last N days
  Future<double> getExpenseForLastNDays(int days) async {
    final db = await database;
    var result = await db.rawQuery(
        "SELECT SUM(amount) as total FROM expenseTable WHERE date >= date('now', '-$days day')");
    return result[0]["total"] != null ? (result[0]["total"] as double) : 0.0;
  }

  // Get the highest expense
  Future<Map<String, dynamic>> getHighestExpense() async {
    final db = await database;
    var result = await db.rawQuery(
        "SELECT category, MAX(amount) as maxAmount FROM expenseTable");
    return result.isNotEmpty ? result.first : {};
  }

  // Get the average expense
  Future<double> getAverageExpense() async {
    final db = await database;
    var result =
    await db.rawQuery("SELECT AVG(amount) as avgExpense FROM expenseTable");
    return result[0]["avgExpense"] != null ? (result[0]["avgExpense"] as double) : 0.0;
  }

  // List all expenses
  Future<List<Map<String, dynamic>>> getAllExpenses() async {
    final db = await database;
    return await db.query('expenseTable', orderBy: 'date DESC');
  }
}
